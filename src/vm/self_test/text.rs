...
pub(crate) fn run_text_lift_test() -> Result<()> {
    ...
    let insts = [
        ...
        Instruction::with2(
            Code::Mov_rm32_r32,
            MemoryOperand::with_base_displ(Register::RSI, 0x40),
            Register::EAX,
        )
        .unwrap(),
        Instruction::with2(
    ...
    let report = analyze_text_lift(&text, base_va, base_va, &[], 0)?;
    ...
    st[interp::STATE_VREGS + 2 * 8..interp::STATE_VREGS + 3 * 8]
        .copy_from_slice(&(b as u64).to_le_bytes());
    st[interp::STATE_VREGS + 8 * 8..interp::STATE_VREGS + 9 * 8]
        .copy_from_slice(&(c as u64).to_le_bytes());
    st[interp::STATE_VREGS + 6 * 8..interp::STATE_VREGS + 7 * 8]
        .copy_from_slice(&(data_off as u64).to_le_bytes()); // rsi
    ...
    let interp_rax = u64::from_le_bytes(
        st[interp::STATE_VREGS + 0 * 8..interp::STATE_VREGS + 1 * 8]
            .try_into()
            .unwrap(),
    );
    ...
    let call_stack_va = vm_arena.base + 0xA000; // dedicated VM bytecode return-IP stack (two-stack)
    {
        ...
        b[0x6000 + interp::STATE_VREGS + 2 * 8..0x6000 + interp::STATE_VREGS + 3 * 8]
            .copy_from_slice(&(b_arg as u64).to_le_bytes());
        b[0x6000 + interp::STATE_VREGS + 8 * 8..0x6000 + interp::STATE_VREGS + 9 * 8]
            .copy_from_slice(&(c as u64).to_le_bytes());
        b[0x6000 + interp::STATE_VREGS + 6 * 8..0x6000 + interp::STATE_VREGS + 7 * 8]
            .copy_from_slice(&(vm_data_va as u64).to_le_bytes()); // rsi
        b[0x6000 + interp::STATE_VREGS + 4 * 8..0x6000 + interp::STATE_VREGS + 5 * 8]
            .copy_from_slice(&((vm_stack_va as u64) + 0xFF8).to_le_bytes());
    ...
    assert_eq!(
        vm_rax, expected,
        "M6 lifted native VM: rax mismatch (vm=0x{:X} native=0x{:X})",
        vm_rax, expected
    );
...
pub(crate) fn run_m6_phase2_lift_test() -> Result<()> {
    ...
    let mut dec =
        iced_x86::Decoder::with_ip(64, &probe.code_buffer, base, iced_x86::DecoderOptions::NONE);
    let mut loop_start = base;
    let mut done_start = base;
    let mut skip_target = base;
    while dec.can_decode() {
        let i = dec.decode();
        if i.code() == Code::Cmp_rm64_r64 {
            loop_start = i.ip();
        }
        if i.code() == Code::Add_rm8_imm8 {
            done_start = i.ip();
        }
        if i.code() == Code::Retnq {
            skip_target = i.ip();
        } // jcxz target = ret (skips add eax,1)
    }
    ...
    let stub = {
        ...
        BlockEncoder::encode(64, blk, BlockEncoderOptions::NONE)
            .map_err(|e| anyhow!("M6-2 native stub encode failed: {}", e))?
            .code_buffer
    };
    {
        let b = narena.bytes();
    ...
    assert_eq!(
        native_rax, want,
        "[23] native reference self-consistency (got {} want {})",
        native_rax, want
    );
    ...
    assert!(
        lift.unsupported.is_empty(),
        "[23] unexpected unsupported {:?}",
        lift.unsupported
    );
    ...
    assert_eq!(
        rax, want,
        "[23] whole-CFG lifted interpreter: rax got {} want {}",
        rax, want
    );
...
pub(crate) fn run_m6_phase2_native_program_test() -> Result<()> {
    ...
    let stub = {
        ...
        BlockEncoder::encode(
            64,
            InstructionBlock::new(&s, ncall as u64),
            BlockEncoderOptions::NONE,
        )
        .map_err(|e| anyhow!("[26] stub encode failed: {}", e))?
        .code_buffer
    };
    {
        let b = narena.bytes();
    ...
    assert_eq!(
        native_rax, want,
        "[26] native reference self-consistency (got {} want {})",
        native_rax, want
    );
    ...
    assert_eq!(
        interp_rax, want,
        "[26] lifted interpreter: rax got {} want {}",
        interp_rax, want
    );
    ...
    let call_stack_va = varena.base + 0xA000; // dedicated VM bytecode return-IP stack (two-stack)
    {
        ...
        b[0x6000 + interp::STATE_VREGS + 4 * 8..][..8]
            .copy_from_slice(&((vsz as u64) + 0xFF8).to_le_bytes());
        b[0x7000..0x7000 + 0x1000].fill(0);
        // Two-stack model: init the dedicated VM return-IP stack and pre-place the
        // outermost return ip (absolute VA of trailing HALT) on it.
        b[0x6000 + interp::STATE_PTR_CALL_STACK..0x6000 + interp::STATE_PTR_CALL_STACK + 8]
    ...
    let b = varena.bytes();
    let vm_rax = u64::from_le_bytes(
        b[0x6000 + interp::STATE_VREGS + 0 * 8..][..8]
            .try_into()
            .unwrap(),
    );
    assert_eq!(
        vm_rax, want,
        "[26] native VM program execution: rax got {} want {}",
        vm_rax, want
    );
