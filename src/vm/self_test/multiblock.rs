...
pub(crate) fn run_m5_multiblock_test() -> Result<()> {
    ...
    eprintln!(
        "[19] blocks={} starts={:?}",
        blocks.len(),
        blocks.iter().map(|b| b.start_va).collect::<Vec<_>>()
    );
    assert!(
        blocks.len() >= 3,
        "M5 CFG expected >=3 blocks, got {}",
        blocks.len()
    );
    ...
    assert_eq!(
        rax, want,
        "M5 lifted interpreter: rax got {} want {}",
        rax, want
    );
...
pub(crate) fn run_switch_lift_test() -> Result<()> {
    ...
    assert!(
        case_vas.iter().all(|&v| v != 0),
        "[24] case blocks not found: {:?}",
        case_vas
    );
    ...
    for (idx, want) in expect.iter().enumerate() {
        ...
        interp::interpret(&mut st, &mut mem, &bc)
            .map_err(|e| anyhow!("[24] interp idx={} failed: {:?}", idx, e))?;
        let rax = u64::from_le_bytes(st[interp::STATE_VREGS + 0 * 8..][..8].try_into().unwrap());
        assert_eq!(
            rax, *want,
            "[24] switch dispatch idx={}: rax got 0x{:X} want 0x{:X}",
            idx, rax, want
        );
    }
