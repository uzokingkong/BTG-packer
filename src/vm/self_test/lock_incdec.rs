...
fn run_case(op: u8, seed_val: u64, w: usize, init_flags: u64) -> Result<(Vec<u8>, u64, Vec<u8>, u64)> {
    ...
    interp::interpret(&mut st, &mut mem, &bc).map_err(|e| anyhow!("lock_incdec interp failed: {:?}", e))?;

    // native
    let (st_n, _vbase, mem_n) = run_native_with_data(&bc, &seed_val.to_le_bytes(), 0, 8, |s, base| {
...
pub(crate) fn run_lock_incdec_test() -> Result<()> {
    ...
    let cases: Vec<(&str, u8, u64, usize, u64, u64, u64)> = vec![
        ...
        ("inc64_keep_cf", OP_LOCK_INC_MEM64_A, 0x7FFF_FFFF_FFFF_FFFF, 8, F_CF | F_AF | F_PF | F_ZF | F_SF | F_OF, 0x8000_0000_0000_0000u64, F_CF | F_SF | F_OF | F_AF | F_PF),
        // inc32: 0xFFFF_FFFF -> 0 (ZF/AF/PF; OF=0, SF=0, no CF — INC never sets CF)
        ("inc32_wrap_zf", OP_LOCK_INC_MEM32_A, 0xFFFF_FFFF, 4, 0, 0, F_ZF | F_AF | F_PF),
        // dec32: 0x8000_0000 -> 0x7FFF_FFFF (OF=1, AF=1, PF=1; SF=0)
        ("dec32_of", OP_LOCK_DEC_MEM32_A, 0x8000_0000, 4, 0, 0x7FFF_FFFF, F_OF | F_AF | F_PF),
        // inc16: 0x00FF -> 0x0100 (AF + PF)
        ("inc16", OP_LOCK_INC_MEM16_A, 0x00FF, 2, 0, 0x0100, F_AF | F_PF),
        // inc8: 0xFF -> 0x00 (ZF/AF/PF; incoming CF preserved)
        ("inc8_wrap_keep_cf", OP_LOCK_INC_MEM8_A, 0xFF, 1, F_CF, 0x00, F_CF | F_ZF | F_AF | F_PF),
        // dec8: 0x00 -> 0xFF (SF/AF/PF; incoming CF preserved)
        ("dec8_wrap_keep_cf", OP_LOCK_DEC_MEM8_A, 0x00, 1, F_CF, 0xFF, F_CF | F_SF | F_AF | F_PF),
    ];
    for (name, op, seed, w, infl, want_v, want_f) in cases {
        let (ib, ifl, nb, nfl) = f(op, seed, w, infl)?;
        let want_b = &want_v.to_le_bytes()[..w];
        assert_eq!(ib, want_b, "{} interp bytes: {:?} != {:?}", name, ib, want_b);
        assert_eq!(nb, want_b, "{} native bytes: {:?} != {:?}", name, nb, want_b);
        assert_eq!(ifl & FLAG_MASK, want_f & FLAG_MASK, "{} interp flags: 0x{:X} != 0x{:X}", name, ifl, want_f);
        assert_eq!(nfl & FLAG_MASK, want_f & FLAG_MASK, "{} native flags: 0x{:X} != 0x{:X}", name, nfl, want_f);
    }
    ...
    // ── lift smoke test: decode real `lock inc/dec [rdi]` bytes ─────────────
    {
        ...
        let (ib, _ifl, nb, _nfl) = {
            ...
            interp::interpret(&mut st, &mut mem, &bc).map_err(|e| anyhow!("lock lift interp failed: {:?}", e))?;
            let (mut st_n, _vbase, mem_n) = run_native_with_data(&bc, &seed.to_le_bytes(), 0, 8, |s, base| {
                set_vreg(s, 7, base + BASE as u64);
            })?;
            set_vreg(&mut st_n, 7, 0);
            (mem[BASE..BASE + 8].to_vec(), 0u64, mem_n[0..8].to_vec(), 0u64)
        };
        assert_eq!(ib, seed.to_le_bytes(), "lift lock inc+dec round-trip (interp)");
        assert_eq!(nb, seed.to_le_bytes(), "lift lock inc+dec round-trip (native)");
    }
