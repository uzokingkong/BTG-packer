...
pub(crate) fn run_a5_sse_cond_test() -> Result<()> {
    ...
    seq.push(LiftedInstr::plain(
        Instruction::with1(Code::Sete_rm8, Register::AL).unwrap(),
    ));
    // cmovcc: cmove ecx, edx  (ZF set -> rcx = rdx)
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Cmove_r64_rm64, Register::R10, Register::RDX).unwrap(),
    ));
    // sbb: r9d = r9d - r8d - CF  (CF=0 from cmp equal)
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Sbb_rm32_r32, Register::R9D, Register::R8D).unwrap(),
    ));
    ...
    seq.push(LiftedInstr::plain(
        Instruction::with2(
            ...
            Register::XMM0,
            MemoryOperand::with_base_displ(Register::RSI, 0x80),
        )
        .unwrap(),
    ));
    seq.push(LiftedInstr::plain(
    ...
    seq.push(LiftedInstr::plain(
        Instruction::with2(
            ...
            Register::XMM6,
            MemoryOperand::with_base_displ(Register::RSI, 0x60),
        )
        .unwrap(),
    ));
    seq.push(LiftedInstr::plain(
        Instruction::with2(
            ...
            MemoryOperand::with_base_displ(Register::RSI, 0x20),
            Register::XMM6,
        )
        .unwrap(),
    ));
    ...
    seq.push(LiftedInstr::plain(
        Instruction::with_stosq(64, iced_x86::RepPrefixKind::Repe).unwrap(),
    ));
    // loopne: rcx-- ; if rcx!=0 && ZF==0 jump to label
    let loop_lbl = 77u32;
    seq.push(LiftedInstr::branch(
        Instruction::with_branch(Code::Loopne_rel8_64_RCX, 0).unwrap(),
        loop_lbl,
    ));
    ...
    mem[rsi + 0x60..rsi + 0x70].copy_from_slice(&[
    ...
    interp::interpret(&mut st, &mut mem, &bc)
        .map_err(|e| anyhow!("A5-sse interp failed: {:?}", e))?;
    ...
    assert_eq!(
        m, 0x1122334455667788,
        "A5-sse: movsd mem copy wrong 0x{:X}",
        m
    );
    // movups copy: [rsi+0x20] holds 16 bytes
    assert_eq!(
        &mem[rsi + 0x20..rsi + 0x30],
        &[
            0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16,
            0x17, 0x18
        ],
        "A5-sse: movups mem copy wrong"
    );
    ...
    assert_eq!(
        rdi_after,
        (rdi + 16) as u64,
        "A5-sse: stosq rdi advance wrong {}",
        rdi_after
    );
    ...
    assert_eq!(
        rcx2, 0xFFFF_FFFF,
        "A5-sse: loopne left rcx={:X} expected 0xFFFFFFFF",
        rcx2
    );
    Ok(())
}
