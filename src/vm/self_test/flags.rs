...
pub(crate) fn run_flags_jcc_test() -> Result<()> {
    ...
    let mut run_prog = |prog: &[u8]| -> (u64, [u64; 16]) {
        ...
        assert_eq!(
            flags_i,
            flags_n,
            "interp vs native flags mismatch (interp=0x{:X} native=0x{:X}) prog:\n{}",
            flags_i,
            flags_n,
    ...
    let conds: [u8; 16] = [
        COND_JE, COND_JNE, COND_JB, COND_JAE, COND_JG, COND_JGE, COND_JL, COND_JLE, COND_JS,
        COND_JNS, COND_JO, COND_JNO, COND_JP, COND_JNP, COND_JA, COND_JBE,
    ];
    for &(a, b) in &pairs {
        ...
        for (i, c) in conds.iter().enumerate() {
            ...
            let expected = if flags::cond_taken(*c, base_flags) {
                1u64
            } else {
                0u64
            };
    ...
    // 3) M2: 64-bit arithmetic + shifts + TEST (interp == native == flags::ref)
    {
        ...
        let pairs64: Vec<(u64, u64)> = (0..12)
            .map(|_| (rng2.next_u64(), rng2.next_u64()))
            .collect();
        for &(a, b) in &pairs64 {
            for (op, expect) in [
        ...
        for _ in 0..40 {
            ...
            let cases: [(u8, u32, u64, u64, u64); 3] = [
                (
                    OP_SHL_R_IMM8,
                    n,
                    x.wrapping_shl(n) as u64,
                    flags::shift_flags(flags::ShiftKind::Shl, x, n, x.wrapping_shl(n)),
                    flags::shift_flags(flags::ShiftKind::Shl, x, n, x.wrapping_shl(n)),
                ),
                (
                    OP_SHR_R_IMM8,
                    n,
                    x.wrapping_shr(n) as u64,
                    flags::shift_flags(flags::ShiftKind::Shr, x, n, x.wrapping_shr(n)),
                    0,
                ),
                (
                    OP_SAR_R_IMM8,
                    ...
                    flags::shift_flags(flags::ShiftKind::Sar, x, n, ((x as i32) >> n) as u32),
                    0,
                ),
            ];
            for (op, cnt, ev, ef, _) in cases {
                ...
                assert_eq!(
                    vregs[0], ev,
                    "shift imm8 op=0x{:02X} x=0x{:X} n={}",
                    op, x, n
                );
                assert_eq!(
                    got & FLAG_MASK,
                    ef & FLAG_MASK,
                    "shift imm8 fl op=0x{:02X} x=0x{:X} n={}",
                    op,
                    x,
                    n
                );
            ...
            for (op, ev, ef) in [
                ...
                assert_eq!(
                    got & FLAG_MASK,
                    ef & FLAG_MASK,
                    "shift cl fl op=0x{:02X} x=0x{:X} n={}",
                    op,
                    x,
                    n
                );
        ...
        for (op, kind) in [
            ...
            assert_eq!(
                got & FLAG_MASK,
                expect & FLAG_MASK,
                "count==0 shift must preserve flags (imm8 0x{:02X})",
                op
            );
        }
        for (op, kind) in [
            ...
            assert_eq!(
                got & FLAG_MASK,
                expect & FLAG_MASK,
                "count==0 shift must preserve flags (cl 0x{:02X})",
                op
            );
        }
        // TEST (rr and imm)
...
pub(crate) fn run_carry_flag_fix_test() -> anyhow::Result<()> {
    ...
    let set_v =
        |s: &mut [u8], i: usize, v: u64| s[vreg(i)..vreg(i) + 8].copy_from_slice(&v.to_le_bytes());
    let get_v =
        |s: &[u8], i: usize| u64::from_le_bytes(s[vreg(i)..vreg(i) + 8].try_into().unwrap());
    ...
    for (cf_in, rax, rbx) in [
        ...
        if got != real {
            return Err(anyhow!(
                "[34] SBB: cf_in={} rax=0x{:X} rbx=0x{:X} got 0x{:X} want 0x{:X}",
                cf_in,
                rax,
                rbx,
                got,
    ...
    for (w, op, a_lo, addend) in [
        (8u64, OP_XADD_MEM8_A, 0xFFu64, 1u64),
        (16u64, OP_XADD_MEM16_A, 0xFFFFu64, 1u64),
    ] {
        let mut bc = BytecodeBuilder::new();
        ...
        if w == 8 {
            ...
            mem[addr..addr + 2].copy_from_slice(&(a_lo as u16).to_le_bytes());
        }
        interp::interpret(&mut st, &mut mem, &code)?;
        let (z, c) = if w == 8 {
            let a = a_lo as u8;
            let s = addend as u8;
            let r = a.wrapping_add(s);
        ...
        if (fl & F_ZF != 0) != z || (fl & F_CF != 0) != c {
            return Err(anyhow!(
                "[34] XADD{} flags wrong: got ZF={} CF={} want ZF={} CF={}",
                w,
                fl & F_ZF != 0,
                fl & F_CF != 0,
                z,
    ...
    // 3) CMPXCHG flag preservation (CF must survive a successful 8-bit CAS)
    {
        ...
        if flags(&st) & F_CF == 0 {
            return Err(anyhow!(
                "[34] CMPXCHG must preserve CF across a successful CAS"
            ));
        }
    }
