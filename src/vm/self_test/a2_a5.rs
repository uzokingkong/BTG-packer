...
pub(crate) fn run_a2_a5_test() -> Result<()> {
    ...
    let mut run_prog = |prog: &[u8]| -> (u64, [u64; 16]) {
        ...
        let b = arena.bytes();
        let sf = 0x6000usize;
        let flags_n = u64::from_le_bytes(
            b[sf + interp::STATE_FLAGS..sf + interp::STATE_FLAGS + 8]
                .try_into()
                .unwrap(),
        );
        let mut vregs_n = [0u64; 16];
        for i in 0..16 {
            vregs_n[i] = u64::from_le_bytes(
                b[sf + interp::STATE_VREGS + i * 8..sf + interp::STATE_VREGS + i * 8 + 8]
                    .try_into()
                    .unwrap(),
            );
        }
        assert_eq!(
            flags_i,
            flags_n,
            "A2 interp vs native flags mismatch\n{}",
            crate::vm::bytecode::disassemble(prog)
        );
        assert_eq!(
            vregs_i,
            vregs_n,
            "A2 interp vs native vregs mismatch\n{}",
            crate::vm::bytecode::disassemble(prog)
        );
        (flags_i, vregs_i)
    };
    ...
    for &(a, b) in &pairs32 {
        ...
        assert_eq!(
            got & FLAG_MASK,
            flags::logical_flags(a | b) & FLAG_MASK,
            "OR32 flags a=0x{:X} b=0x{:X}",
            a,
            b
        );
    ...
    for &(a, b) in &pairs64 {
        ...
        assert_eq!(
            got & FLAG_MASK,
            flags::logical_flags64(a | b) & FLAG_MASK,
            "OR64 flags"
        );
    ...
    for &(a, _) in &pairs64 {
        ...
        assert_eq!(
            got & FLAG_MASK,
            flags::sub_flags64(0, a) & FLAG_MASK,
            "NEG64 flags a=0x{:X}",
            a
        );
    }
    ...
    for _ in 0..24 {
        ...
        for (op, ev, ef) in [
            (
                OP_SHL64_R_IMM8,
                x.wrapping_shl(n as u32) as u64,
                flags::shift_flags64(flags::ShiftKind::Shl, x, n as u32, x.wrapping_shl(n as u32)),
            ),
            (
                OP_SHR64_R_IMM8,
                x.wrapping_shr(n as u32) as u64,
                flags::shift_flags64(flags::ShiftKind::Shr, x, n as u32, x.wrapping_shr(n as u32)),
            ),
            (
                OP_SAR64_R_IMM8,
                ((x as i64) >> n) as u64,
                flags::shift_flags64(flags::ShiftKind::Sar, x, n as u32, ((x as i64) >> n) as u64),
            ),
        ] {
            let mut bc = BytecodeBuilder::new();
            bc.mov_r_imm64(0, x);
            ...
            assert_eq!(
                got & FLAG_MASK,
                ef & FLAG_MASK,
                "SHL64 imm8 flags op=0x{:02X} x=0x{:X} n={}",
                op,
                x,
                n
            );
        ...
        for (op, ev, ef) in [
            ...
            assert_eq!(
                got & FLAG_MASK,
                ef & FLAG_MASK,
                "SHL64 CL flags op=0x{:02X} x=0x{:X} n={}",
                op,
                x,
                n
            );
...

    {
        ...
        seq.push(LiftedInstr::plain(
            Instruction::with2(Code::Or_rm32_r32, Register::EAX, Register::EDX).unwrap(),
        ));
        seq.push(LiftedInstr::plain(
            Instruction::with1(Code::Neg_rm32, Register::EAX).unwrap(),
        ));
        seq.push(LiftedInstr::plain(
            Instruction::with2(Code::Shl_rm64_imm8, Register::RAX, 3).unwrap(),
        ));
        ...
        assert!(
            bad.is_empty(),
            "A5 diagnose: unexpected unsupported {:?}",
            bad
        );
        ...
        bad_seq.push(LiftedInstr::plain(
            Instruction::with2(Code::Sqrtss_xmm_xmmm32, Register::XMM0, Register::XMM1).unwrap(),
        ));
...
pub(crate) fn run_a2_lift_completion_test() -> Result<()> {
    ...
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Add_rm32_imm8, Register::EAX, 0x20).unwrap(),
    )); // eax += 0x20 (imm8)
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Movsxd_r64_rm32, Register::RDX, Register::EAX).unwrap(),
    )); // rdx = sext(eax)
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Sub_rm64_imm8, Register::RDX, 0x10).unwrap(),
    )); // rdx -= 0x10 (imm8 -> add -0x10)
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::And_rm64_imm8, Register::RDX, 0x3F).unwrap(),
    )); // rdx &= 0x3F (imm8)
    seq.push(LiftedInstr::plain(
        Instruction::with2(
            Code::Lea_r32_m,
            Register::R8D,
            MemoryOperand::with_base_displ(Register::RCX, -1),
        )
        .unwrap(),
    )); // r8 = rcx - 1
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Movzx_r32_rm8, Register::EDI, Register::CL).unwrap(),
    )); // edi = cl (movzx reg)
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Cmp_rm64_r64, Register::RAX, Register::RDX).unwrap(),
    )); // cmp rax, rdx (full flags)
    seq.push(LiftedInstr::branch(
        Instruction::with_branch(Code::Jg_rel32_64, 0).unwrap(),
        SKIP,
    )); // jg SKIP (taken: rax>rdx)
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Mov_r32_imm32, Register::EAX, 0).unwrap(),
    )); // eax = 0 (skipped)
    seq.push(LiftedInstr::labeled(
        Instruction::with1(Code::Push_r64, Register::RDX).unwrap(),
        SKIP,
    )); // SKIP: push rdx
    seq.push(LiftedInstr::plain(
        Instruction::with1(Code::Pop_r64, Register::R9).unwrap(),
    )); // pop r9 (r9 = rdx)
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Test_rm64_r64, Register::RBX, Register::RBX).unwrap(),
    )); // test rbx, rbx
    seq.push(LiftedInstr::plain(Instruction::with(Code::Cdqe))); // rax = sext(eax)
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Add_rm32_imm8, Register::EAX, 5).unwrap(),
    )); // eax += 5
    seq.push(LiftedInstr::plain(Instruction::with(Code::Retnq)));
    ...
    assert!(
        bad.is_empty(),
        "A2-lift-completion: unexpected unsupported {:?}",
        bad
    );
    ...
    st[interp::STATE_VREGS + 0 * 8..interp::STATE_VREGS + 1 * 8]
        .copy_from_slice(&0u64.to_le_bytes()); // rax
    st[interp::STATE_VREGS + 1 * 8..interp::STATE_VREGS + 2 * 8]
        .copy_from_slice(&(a as u64).to_le_bytes()); // rcx = a
    st[interp::STATE_VREGS + 3 * 8..interp::STATE_VREGS + 4 * 8]
        .copy_from_slice(&0u64.to_le_bytes()); // rbx = 0
    st[interp::STATE_VREGS + 4 * 8..interp::STATE_VREGS + 5 * 8]
        .copy_from_slice(&0x3FF8u64.to_le_bytes()); // vreg4 = RSP (stack top)
    ...
    mem[(interp::CALL_STACK_SIZE - 8) as usize..interp::CALL_STACK_SIZE]
        .copy_from_slice(&halt_off.to_le_bytes());
    interp::interpret(&mut st, &mut mem, &bc)
        .map_err(|e| anyhow!("A2-lift-completion interp failed: {:?}", e))?;
    let rax = u64::from_le_bytes(
        st[interp::STATE_VREGS + 0 * 8..interp::STATE_VREGS + 1 * 8]
            .try_into()
            .unwrap(),
    );
    let r9 = u64::from_le_bytes(
        st[interp::STATE_VREGS + 9 * 8..interp::STATE_VREGS + 10 * 8]
            .try_into()
            .unwrap(),
    );
    assert_eq!(
        rax, expected_rax,
        "A2-lift-completion: rax mismatch (got 0x{:X} want 0x{:X})",
        rax, expected_rax
    );
    assert_eq!(
        r9, expected_r9,
        "A2-lift-completion: r9 mismatch (got 0x{:X} want 0x{:X})",
        r9, expected_r9
    );
    Ok(())
}
...
pub(crate) fn run_a2a5_lift_residual_test() -> Result<()> {
    ...
    let mut seq: Vec<LiftedInstr> = Vec::new();
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Mov_r32_imm32, Register::EAX, 0x10).unwrap(),
    ));
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Add_rm8_imm8, Register::AL, 0x05).unwrap(),
    )); // al=0x15
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Mov_r32_imm32, Register::EBX, 0x0F).unwrap(),
    ));
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Add_rm8_r8, Register::AL, Register::BL).unwrap(),
    )); // al=0x24
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Sub_rm8_imm8, Register::AL, 0x01).unwrap(),
    )); // al=0x23
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Mov_r32_imm32, Register::ECX, 0x1234).unwrap(),
    ));
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Add_rm16_imm16, Register::CX, 0x0021).unwrap(),
    )); // cx=0x1255
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Mov_r32_imm32, Register::EDX, 0x0100).unwrap(),
    ));
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Sub_rm16_r16, Register::CX, Register::DX).unwrap(),
    )); // cx=0x1155
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Xor_rm8_imm8, Register::AL, 0x0F).unwrap(),
    )); // al=0x2C
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::And_rm16_imm16, Register::CX, 0x00FF).unwrap(),
    )); // cx=0x0055
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Or_rm8_imm8, Register::AL, 0x80).unwrap(),
    )); // al=0xAC
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Mov_r32_imm32, Register::R8D, 0x00FF).unwrap(),
    ));
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Or_rm16_imm16, Register::R8W, 0x0F00).unwrap(),
    )); // r8w=0x0FFF
    ...
    let skip = 50u32;
    seq.push(LiftedInstr::branch(
        Instruction::with_branch(Code::Jrcxz_rel8_64, 0).unwrap(),
        skip,
    ));
    seq.push(LiftedInstr::plain(
        Instruction::with2(Code::Add_rm32_imm8, Register::EAX, 1).unwrap(),
    )); // skipped
    ...
    mem[(interp::CALL_STACK_SIZE - 8) as usize..interp::CALL_STACK_SIZE]
        .copy_from_slice(&halt_off.to_le_bytes());
    interp::interpret(&mut st, &mut mem, &bc)
        .map_err(|e| anyhow!("[21] interp failed: {:?}", e))?;
    ...
    assert_eq!(
        rax & 0xFF,
        0xAC,
        "[21] 8-bit arithmetic al mismatch (got 0x{:X})",
        rax & 0xFF
    );
    assert_eq!(
        rax & 0xFFFF_FFFF,
        0xAC,
        "[21] jcxz should skip the add (rax=0x{:X})",
        rax
    );
    assert_eq!(
        rcx & 0xFFFF,
        0x00,
        "[21] rcx should be 0 after jcxz (got 0x{:X})",
        rcx
    );
    assert_eq!(
        r8 & 0xFFFF,
        0x0FFF,
        "[21] 16-bit or mismatch (r8w=0x{:X})",
        r8 & 0xFFFF
    );
    ...
    for i in 0..count {
        mm[src + (i as usize) * 4..src + (i as usize) * 4 + 4]
            .copy_from_slice(&((i as u32) * 0x11223344).to_le_bytes());
    }
    ...
    stm[interp::STATE_PTR_STACK..interp::STATE_PTR_STACK + 8]
    ...
    interp::interpret(&mut stm, &mut mm, &mbc)
        .map_err(|e| anyhow!("[21] rep movsd interp failed: {:?}", e))?;
    for i in 0..count {
        let expect = (i as u32) * 0x11223344;
        let got = u32::from_le_bytes(
            mm[dst + (i as usize) * 4..dst + (i as usize) * 4 + 4]
                .try_into()
                .unwrap(),
        );
        assert_eq!(
            got, expect,
            "[21] rep movsd copy[{}] mismatch (got 0x{:X})",
            i, got
        );
    }
    ...
    let run_cmps = |a_off: usize, b_off: usize, n: u64, mut mem_data: &mut [u8]| -> u64 {
        ...
        stc[interp::STATE_PTR_STACK..interp::STATE_PTR_STACK + 8]
            .copy_from_slice(&0x3000u64.to_le_bytes());
        stc[interp::STATE_SP..interp::STATE_SP + 8].copy_from_slice(&0xFF8u64.to_le_bytes());
        mem_data[0x3000 + 0xFF8..0x3000 + 0x1000]
            .copy_from_slice(&((cbc.len() - 1) as u64).to_le_bytes());
    ...
    for i in 0..4usize {
        mc[0x3000 + i * 4..0x3000 + i * 4 + 4]
            .copy_from_slice(&((i as u32) * 0x11111111).to_le_bytes());
        mc[0x3100 + i * 4..0x3100 + i * 4 + 4]
            .copy_from_slice(&((i as u32) * 0x11111111).to_le_bytes());
    }
    let rem_match = run_cmps(0x3000, 0x3100, 4, &mut mc);
    assert_eq!(
        rem_match, 0,
        "[21] rep cmpsd match: rcx should reach 0 (got {})",
        rem_match
    );
    ...
    assert_eq!(
        rem_mis, 2,
        "[21] rep cmpsd mismatch at elem1: rcx should be 2 (got {})",
        rem_mis
    );
