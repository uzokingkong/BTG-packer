...
fn bridge_noop_preserves_full_state() {
    ...
    assert_eq!(empty, bridged, "NativeCallBridge must not alter any VM state");
}
...
fn bridge_mid_program_is_transparent() {
    ...
    let no_bridge = RiscProgram::new(vec![
        MicroInstr::new(Mov).with_dst(MicroOperand::VReg(0)).with_imm(0x1234),
        MicroInstr::new(Mov).with_dst(MicroOperand::VReg(1)).with_imm(0x5678),
        MicroInstr::new(Nor)
    ...
    assert_eq!(a, b, "injected NativeCallBridge must be semantically transparent");
}
...
fn reentrant_callback_preserves_outer_vm_state() {
    ...
    sub.emit_add(MicroOperand::VReg(0), MicroOperand::Imm64(0xDEAD), MicroOperand::Imm64(0));
    sub.emit_add(MicroOperand::VReg(5), MicroOperand::Imm64(0x11), MicroOperand::Imm64(0));
    ...
    outer.emit_add(MicroOperand::VReg(1), MicroOperand::Imm64(0xCAFE), MicroOperand::Imm64(0));
    outer.emit_push(MicroOperand::VReg(1));
    outer.emit_add(MicroOperand::VReg(2), MicroOperand::Imm64(0x777), MicroOperand::Imm64(0));
    outer.emit_push(MicroOperand::VReg(2));
    outer.instrs.push(MicroInstr::new(RiscOp::VmCallBridge).with_imm(7));
    outer.emit_pop(MicroOperand::VReg(3));
    outer.emit_pop(MicroOperand::VReg(4));
    outer.instrs.push(
        MicroInstr::new(RiscOp::Mov).with_dst(MicroOperand::VReg(5)).with_src1(MicroOperand::VReg(0)),
    );
    ...
    plain.emit_add(MicroOperand::VReg(1), MicroOperand::Imm64(0xCAFE), MicroOperand::Imm64(0));
    plain.emit_push(MicroOperand::VReg(1));
    plain.emit_add(MicroOperand::VReg(2), MicroOperand::Imm64(0x777), MicroOperand::Imm64(0));
    ...
    assert_eq!(st.regs[3], 0x777, "outer stack top preserved across callback re-entry");
    assert_eq!(st.regs[4], 0xCAFE, "outer second frame preserved across callback re-entry");
    ...
    assert_eq!(st.regs[5], 0xDEAD, "caller copies callback return after re-entry");
    for i in 1..16 {
        if i == 5 {
            continue; // callback mutated regs[5]
        }
        assert_eq!(st.regs[i], stp.regs[i], "outer vreg {} identical with/without callback", i);
    }
    assert_eq!(st.flags, stp.flags, "outer flags preserved across callback re-entry");
    assert_eq!(st.vsp, stp.vsp, "outer virtual stack pointer preserved");
    assert_eq!(st.stack, stp.stack, "outer virtual stack contents preserved");
}
...
fn doubly_nested_callback_preserves_outer_state() {
    ...
    inn.emit_add(MicroOperand::VReg(0), MicroOperand::Imm64(0x1234), MicroOperand::Imm64(0));
    ...
    mid.instrs.push(MicroInstr::new(RiscOp::VmCallBridge).with_imm(9));
    mid.emit_add(MicroOperand::VReg(0), MicroOperand::VReg(0), MicroOperand::Imm64(1));
    ...
    let mut outer = crate::vm::risc::RiscDesynthesizer::new();
    outer.emit_add(MicroOperand::VReg(1), MicroOperand::Imm64(0xABCD), MicroOperand::Imm64(0));
    outer.emit_push(MicroOperand::VReg(1));
    outer.emit_add(MicroOperand::VReg(2), MicroOperand::Imm64(0x5555), MicroOperand::Imm64(0));
    outer.instrs.push(MicroInstr::new(RiscOp::VmCallBridge).with_imm(8));
    outer.emit_pop(MicroOperand::VReg(3));
    outer.instrs.push(MicroInstr::new(RiscOp::Halt));
    let outer_prog = RiscProgram::new(outer.instrs);
    ...
    assert_eq!(st.regs[0], 0x1235, "nested callback chain return (0x1234+1)");
    assert_eq!(st.regs[3], 0xABCD, "outer stack frame preserved across double nesting");
    assert_eq!(st.regs[2], 0x5555, "outer live register preserved across double nesting");
}
...
fn vtable_indirect_dispatch_at_boundary() {
    ...
    let prog = RiscProgram::new(vec![
        MicroInstr::new(RiscOp::Mov).with_dst(MicroOperand::VReg(0)).with_imm(0), // RAX=0
        MicroInstr::new(RiscOp::VirtualBranch { cond: crate::vm::risc::BranchCondition::Always })
    ...
    assert_eq!(st_a.flags & crate::vm::risc::VFLAG_CF, 0, "callee A add is carry-free");
    assert_eq!(st_b.flags & crate::vm::risc::VFLAG_CF, 0, "callee B add is carry-free");
}
