...
fn hdr_labeled(
    seq: &mut Vec<(Instruction, Option<Cl>)>,
    op: u8,
    body: Vec<(Instruction, Option<Cl>)>,
) {
...
pub(crate) fn emit_shift_imm8_32(seq: &mut Vec<(Instruction, Option<Cl>)>) {
    for (op, code) in [
        ...
        let mut body: Vec<(Instruction, Option<Cl>)> = vec![
            ...
            (
                Instruction::with2(Code::Test_rm32_r32, Register::ECX, Register::ECX).unwrap(),
                None,
            ),
            (
                Instruction::with_branch(Code::Je_rel32_64, 0).unwrap(),
                Some(Cl::ShiftSkip(op)),
            ),
            (
...
pub(crate) fn emit_shld_shrd(seq: &mut Vec<(Instruction, Option<Cl>)>) {
    ...
    for (op, is64, is_shld, has_imm) in [
        ...
        let (dst, src) = if is64 {
            (Register::R11, Register::RDX)
        } else {
            (Register::R11D, Register::EDX)
        };
        ...
        let mut body: Vec<(Instruction, Option<Cl>)> = vec![
            ...
            (
                Instruction::with2(mv, dst, vreg(Register::RCX)).unwrap(),
                None,
            ), // dst value
            (
                Instruction::with2(mv, src, vreg(Register::RDX)).unwrap(),
                None,
            ), // src value
        ];
        if has_imm {
        ...
        body.push((
            Instruction::with2(Code::And_rm32_imm32, Register::ECX, cmask).unwrap(),
            None,
        ));
        body.push((
            Instruction::with2(Code::Test_rm32_r32, Register::ECX, Register::ECX).unwrap(),
        ...
        body.push((
            Instruction::with3(shc, dst, src, Register::CL).unwrap(),
            None,
        ));
