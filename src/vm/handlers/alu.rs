...
// ── v52: BMI1/2 (Group B) register-register handlers ───────────────────────
...
// interpreter); BLSR/BLSMSK/BLSI/ANDN are portable bit arithmetic (no flags).
...
// LZCNT r32/r64
pub(super) fn emit_lzcnt(seq: &mut Vec<(Instruction, Option<Cl>)>) {
    for (op, code, is64) in [
...
// POPCNT r32/r64
pub(super) fn emit_popcnt(seq: &mut Vec<(Instruction, Option<Cl>)>) {
    for (op, code, is64) in [
...
pub(super) fn emit_blsr(seq: &mut Vec<(Instruction, Option<Cl>)>) {
...
pub(super) fn emit_blsmsk(seq: &mut Vec<(Instruction, Option<Cl>)>) {
...
pub(super) fn emit_blsi(seq: &mut Vec<(Instruction, Option<Cl>)>) {
...
pub(super) fn emit_andn(seq: &mut Vec<(Instruction, Option<Cl>)>) {
