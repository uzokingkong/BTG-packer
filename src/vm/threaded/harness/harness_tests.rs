...
// ==============================================================================
    ...
    /// P1 (??: ADD MBA ??desynth.emit_add ??`AddWithCarry` �?만들므�????�스?�는
    /// AddWithCarry ?�들??== 참조 eval_state �?검증한?? 진짜 `RiscOp::Add{width:8}`
    /// MBA 경로(?�래�??�확???�함)??`test_mba_add_handler_diversified_per_key`
    /// �?검증한??
    #[test]
    fn test_native_mba_add_matches_reference_state() {
        let mut d = RiscDesynthesizer::new();
        // R0 = 0xFFFF_FFFF_FFFF_FFFF
        d.emit_add(MicroOperand::VReg(0), MicroOperand::Imm64(0xFFFF_FFFF_FFFF_FFFF), MicroOperand::Imm64(0));
        // R1 = 1
        d.emit_add(MicroOperand::VReg(1), MicroOperand::Imm64(1), MicroOperand::Imm64(0));
        // R2 = R0 + R1  ??0, CF=1 (?�버?�로 캐리)
        d.emit_add(MicroOperand::VReg(2), MicroOperand::VReg(0), MicroOperand::VReg(1));
        // R3 = 0x7FFF_FFFF_FFFF_FFFF, R4 = 1 ??OF=1 (부???�버?�로)
        d.emit_add(MicroOperand::VReg(3), MicroOperand::Imm64(0x7FFF_FFFF_FFFF_FFFF), MicroOperand::Imm64(0));
        d.emit_add(MicroOperand::VReg(4), MicroOperand::Imm64(1), MicroOperand::Imm64(0));
        d.emit_add(MicroOperand::VReg(5), MicroOperand::VReg(3), MicroOperand::VReg(4));
        // R6 = 0x1234_5678 + 0x8765_4321 = 0x9999_9999 (ZF=0, SF=1)
        d.emit_add(MicroOperand::VReg(6), MicroOperand::Imm64(0x1234_5678_0000_0000), MicroOperand::Imm64(0));
        d.emit_add(MicroOperand::VReg(7), MicroOperand::Imm64(0x8765_4321_0000_0000), MicroOperand::Imm64(0));
        d.emit_add(MicroOperand::VReg(8), MicroOperand::VReg(6), MicroOperand::VReg(7));
        d.instrs.push(MicroInstr::new(RiscOp::Halt));
        let prog = RiscProgram::new(d.instrs);

        // 모든 64-bit Add가 MBA 경로�??�?�록 강제.
        ...
        assert_eq!(nat.flags, ref_st.flags,
            "MBA add flags mismatch (ref={:#x} nat={:#x})", ref_st.flags, nat.flags);
    ...
    /// P1 (??: mba_prob=100/0 강제 ??결과 ?�치 ??**AddWithCarry 경로**(desynth.emit_add
    /// 가 만드??op) 검증이?? ?�제 `RiscOp::Add{width:8}` ?�들??MBA 경로??
    /// `test_mba_add_handler_diversified_per_key` 가 커버?�다 (?�기???�치/?�래�?
    /// ?�치 + 결정???�시 ?�택???�실???�정???�인).
    #[test]
    fn test_native_mba_add_prob_0_vs_100_equivalent() {
        let mut d = RiscDesynthesizer::new();
        let vals: [u64; 8] = [0, 1, 0xFFFFFFFF, 0x8000_0000_0000_0000, 0xDEAD_BEEF, 0x7FFF_FFFF_FFFF_FFFF, 0x0000_0001_0000_0000, 0xAAAA_BBBB_CCCC_DDDD];
        for (i, v) in vals.iter().enumerate() {
            d.emit_add(MicroOperand::VReg(i as u8), MicroOperand::Imm64(*v), MicroOperand::Imm64(0));
        }
        // R0+R1, R2+R3, R4+R5, R6+R7 (?�버?�로 ?�함 4??
        d.emit_add(MicroOperand::VReg(0), MicroOperand::VReg(0), MicroOperand::VReg(1));
        d.emit_add(MicroOperand::VReg(2), MicroOperand::VReg(2), MicroOperand::VReg(3));
        d.emit_add(MicroOperand::VReg(4), MicroOperand::VReg(4), MicroOperand::VReg(5));
        d.emit_add(MicroOperand::VReg(6), MicroOperand::VReg(6), MicroOperand::VReg(7));
        d.instrs.push(MicroInstr::new(RiscOp::Halt));
        let prog = RiscProgram::new(d.instrs);
        ...
        assert_eq!(a.flags, b.flags, "MBA vs plain add flags differ (plain={:#x} mba={:#x})", a.flags, b.flags);
    }

    /// P1 (handler diversification): ?�제 `RiscOp::Add { width: 8 }` (x86 ADD??
    /// P0-1 ?�래�??�확 op ??desynth.emit_add ??AddWithCarry �??��?�??�전 MBA
    /// ?�스?�들?� MBA 경로�??�제�??�지 ?�았??�??�함???�로그램?�로,
    ///
    ///  (a) mba_prob=100 강제 ???�이?�브 ?�행??참조 `eval_state`?� ?�확???�치?�고,
    ///  (b) **빌드 ?��? ?�르�?* ADD ?�들??코드(variant 0 `xor r9,r11` vs
    ///      variant 1 `or r9,r11`)가 ?�르�?emit ?�다 (빌드 �?handler ?�형??.
    #[test]
    fn test_mba_add_handler_diversified_per_key() {
        use crate::vm::threaded::harness::OFF_CODE;
        // 8개의 64-bit ADD: R(i) += R((i+1)%8) ???�버?�로/캐리/부???�래�??�함.
        let mut instrs = Vec::new();
        let vals: [u64; 8] = [
            0xFFFF_FFFF_FFFF_FFFF, 1,
            0x7FFF_FFFF_FFFF_FFFF, 1,
            0x1234_5678_0000_0000, 0x8765_4321_0000_0000,
        ...
        let signature = |vm: &mut NativeVmHarness| -> String {
            ...
            let mut i = 0;
            while i + 3 <= bytes.len() {
                if bytes[i..i + 3] == [0x4D, 0x33, 0xCB] { s.push('X'); i += 3; }
                else if bytes[i..i + 3] == [0x4D, 0x0B, 0xCB] { s.push('O'); i += 3; }
                else { i += 1; }
            }
            s
        };
        ...
        assert!(sa.len() >= 8, "key=0x11: all 8 ADD blocks must use an MBA variant ({sa})");
        assert!(sb.len() >= 8, "key=0x22: all 8 ADD blocks must use an MBA variant ({sb})");
        // ??빌드가 블록�?variant 배정???�라???�다 (같�? ?�로그램, ?�른 ?????�른 ?�들??코드).
        assert!(
            sa != sb,
    ...
    // ?�?� P0-2: 3-path randomized differential ??eval_state vs poly vs native ?�?�
    // 보고??P0 "VM/native differential execution 강화": 같�? ?�덤 RISC ?�로그램??
    // (1) 참조 eval_state, (2) ?�리 ?�터?�리??롤링??, (3) ?�이?�브 threaded
    // ?�네????경로�??�행?�고 **?�체 ?�태**(regs/temps/flags/vsp/stack)가 ?�치?�는지
    // 무작?�로 검증한?? mem op ???�이?�브 ?�네?��? 가??arena 주소�??��?�?
    // (eval_state ??HashMap �?계약???�름) 비교 범위?�서 ?�외?�다.
    #[test]
    fn p0_randomized_three_path_differential() {
        ...
        for trial in 0..64 {
            ...
            // �??�택 pop(?�더?�로)?� 참조/?�이?�브 계약???�르므�?균형 push/pop �?
            let mut depth = 0i32;
            for _ in 0..n {
                ...
                if choice == 11 && depth <= 0 {
                    choice = 10; // �??�택?�면 pop ?�??push
                }
                match choice {
                    0 => d.emit_add(MicroOperand::VReg(ra), MicroOperand::Imm64(rng.gen::<u64>()), MicroOperand::Imm64(0)),
                    1 => d.emit_add(MicroOperand::VReg(ra), MicroOperand::VReg(rb), MicroOperand::VReg(rc)),
                    2 => d.emit_sub(MicroOperand::VReg(ra), MicroOperand::VReg(rb), MicroOperand::VReg(rc)),
                    3 => d.emit_xor(MicroOperand::VReg(ra), MicroOperand::VReg(rb), MicroOperand::VReg(rc)),
                    4 => d.emit_and(MicroOperand::VReg(ra), MicroOperand::VReg(rb), MicroOperand::VReg(rc)),
                    5 => d.emit_or(MicroOperand::VReg(ra), MicroOperand::VReg(rb), MicroOperand::VReg(rc)),
                    6 => d.instrs.push(MicroInstr::new(RiscOp::Nor).with_dst(MicroOperand::VReg(ra)).with_src1(MicroOperand::VReg(rb)).with_src2(MicroOperand::VReg(rc))),
                    7 => d.instrs.push(MicroInstr::new(RiscOp::ShiftLeft).with_dst(MicroOperand::VReg(ra)).with_src1(MicroOperand::VReg(rb)).with_src2(MicroOperand::Imm64((rng.gen::<u8>() % 63 + 1) as u64))),
                    8 => d.instrs.push(MicroInstr::new(RiscOp::ShiftRight).with_dst(MicroOperand::VReg(ra)).with_src1(MicroOperand::VReg(rb)).with_src2(MicroOperand::Imm64((rng.gen::<u8>() % 63 + 1) as u64))),
                    9 => d.instrs.push(MicroInstr::new(RiscOp::ArithmeticShiftRight).with_dst(MicroOperand::VReg(ra)).with_src1(MicroOperand::VReg(rb)).with_src2(MicroOperand::Imm64((rng.gen::<u8>() % 63 + 1) as u64))),
                    10 => { d.emit_push(MicroOperand::VReg(ra)); depth += 1; }
                    11 => { d.emit_pop(MicroOperand::VReg(ra)); depth -= 1; }
                    12 => d.instrs.push(MicroInstr::new(RiscOp::SetFlag).with_src1(MicroOperand::Imm64(rng.gen::<u64>() & 0x8D5))),
                    13 => d.instrs.push(MicroInstr::new(RiscOp::Add { width: 4 }).with_dst(MicroOperand::VReg(ra)).with_src1(MicroOperand::VReg(rb)).with_src2(MicroOperand::VReg(rc))),
                    14 => d.instrs.push(MicroInstr::new(RiscOp::Add { width: 1 }).with_dst(MicroOperand::VReg(ra)).with_src1(MicroOperand::VReg(rb)).with_src2(MicroOperand::VReg(rc))),
                    15 => d.instrs.push(MicroInstr::new(RiscOp::Not { width: 8 }).with_dst(MicroOperand::VReg(ra)).with_src1(MicroOperand::VReg(rb))),
            ...
            // (2) ?�리
            let mut enc = PolymorphicEncoder::new(seed);
            let bc = enc.encode(&prog).expect("all random ops must be poly-encodable");
            let mut interp = PolymorphicInterpreter::new(seed);
            interp.regs = init;
            ...
            // ?�체 ?�태 비교 (mem ?�외 ???�이?�브??가??arena 주소 계약).
            assert_eq!(interp.regs, ref_st.regs, "trial={trial} poly regs != ref\nprog:\n{:?}", prog.instrs);
            assert_eq!(interp.temps, ref_st.temps, "trial={trial} poly temps != ref");
            assert_eq!(interp.flags.raw, ref_st.flags, "trial={trial} poly flags != ref (ref={:#x} poly={:#x})", ref_st.flags, interp.flags.raw);
            assert_eq!(interp.vsp, ref_st.vsp, "trial={trial} poly vsp != ref");
            assert_eq!(interp.stack, ref_st.stack, "trial={trial} poly stack != ref");
            assert_eq!(nat.regs, ref_st.regs, "trial={trial} native regs != ref");
            assert_eq!(nat.temps, ref_st.temps, "trial={trial} native temps != ref");
            assert_eq!(nat.flags, ref_st.flags, "trial={trial} native flags != ref (ref={:#x} nat={:#x})\nprog:\n{:?}", ref_st.flags, nat.flags, prog.instrs);
            assert_eq!(nat.vsp, ref_st.vsp, "trial={trial} native vsp != ref (ref={:#x} nat={:#x})\nprog:\n{:?}", ref_st.vsp, nat.vsp, prog.instrs);
