...
pub(super) fn two_op(
    ...
    let is64_code = matches!(
        inst.code(),
        iced_x86::Code::Add_rm64_r64
            | iced_x86::Code::Sub_rm64_r64
            | iced_x86::Code::Xor_rm64_r64
            | iced_x86::Code::And_rm64_r64
            | iced_x86::Code::Or_rm64_r64
            | iced_x86::Code::Imul_r64_rm64
            | iced_x86::Code::Add_r64_rm64
            | iced_x86::Code::Sub_r64_rm64
            | iced_x86::Code::Xor_r64_rm64
            | iced_x86::Code::And_r64_rm64
            | iced_x86::Code::Or_r64_rm64
            | iced_x86::Code::Imul_r64_rm64
    );
    let mut d = 0u8;
    ...
    if mem_dst {
        ...
        if is64_code {
            b.mem_load_a(OP_MOV_R_MEM64_A, SCRATCH2, mem_addr);
        } else {
            b.mem_load_a(OP_MOVZX_R_MEM32_A, SCRATCH2, mem_addr);
        }
        d = SCRATCH2;
    } else {
...
pub(super) fn lift_arith_imm(b: &mut BytecodeBuilder, inst: &Instruction) -> Result<()> {
    ...
    if inst.op0_kind() == OpKind::Register {
        ...
        if sz == 64 {
            b.binop_r_imm64(op64, SCRATCH2, imm as u32);
        } else {
            b.binop_r_imm32(op32, SCRATCH2, imm as u32);
        }
...
pub(super) fn lift_narrow_arith(b: &mut BytecodeBuilder, inst: &Instruction) -> Result<()> {
    ...
    let mem_dst = inst.op0_kind() == OpKind::Memory;
    let mem_addr = if mem_dst {
        Some(mem_emit(b, inst, 0)?)
    } else {
        None
    };
    if mem_dst {
    ...
    let (val_r, out) = if mem_dst {
        (SCRATCH2, mem_addr.unwrap())
    } else {
        (SCRATCH, 0u8)
    };
