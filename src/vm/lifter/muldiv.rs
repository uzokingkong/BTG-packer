...
pub(super) fn lift_bt(b: &mut BytecodeBuilder, inst: &Instruction) -> Result<()> {
    ...
    let wbits = if name.contains("rm16_") {
        16
    ...
    if inst.op1_kind() == OpKind::Register {
        ...
        if wbits == 64 {
            b.mov_r_r64(SCRATCH2, idx);
        } else {
            b.mov_r_r(SCRATCH2, idx);
        }
        ...
        b.jcc8(COND_JE, done);
        if wbits == 64 {
            b.shift64_r_imm8(OP_SHR64_R_IMM8, SCRATCH, 1);
        } else {
            b.shift_r_imm8(OP_SHR_R_IMM8, SCRATCH, 1);
        }
        b.dec_r(SCRATCH2);
        b.jmp8(loop_lbl);
        b.mark_label(done);
        ...
        let cnt = if wbits == 64 {
            inst.immediate8() & 0x3F
        } else {
            inst.immediate8() & 0x1F
        };
        if wbits == 64 {
            b.shift64_r_imm8(OP_SHR64_R_IMM8, SCRATCH, cnt);
        } else {
            b.shift_r_imm8(OP_SHR_R_IMM8, SCRATCH, cnt);
        }
    }
...
pub(super) fn lift_bts(b: &mut BytecodeBuilder, inst: &Instruction) -> Result<()> {
    ...
    if wbits == 64 {
        b.shift64_r_imm8(OP_SHL64_R_IMM8, TMP, 1);
    } else {
        b.shift_r_imm8(OP_SHL_R_IMM8, TMP, 1);
    }
    b.dec_r(SCRATCH2);
    b.jmp8(loop_lbl);
    ...
    if is_bts {
        if wbits == 64 {
            b.binop_r_r64(OP_OR_R_R64, dst_r, TMP);
        } else {
            b.binop_r_r(OP_OR_R_R, dst_r, TMP);
        }
    } else if is_btr {
        if wbits == 64 {
            b.not_r64(TMP);
            b.binop_r_r64(OP_AND_R_R64, dst_r, TMP);
        } else {
            b.not_r(TMP);
            b.binop_r_r(OP_AND_R_R, dst_r, TMP);
        }
    } else {
        if wbits == 64 {
            b.binop_r_r64(OP_XOR_R_R64, dst_r, TMP);
        } else {
            b.binop_r_r(OP_XOR_R_R, dst_r, TMP);
        }
    }

    if let Some(addr) = mem_addr {
        let store = match wbits {
            16 => OP_MOV_MEM16_A,
            64 => OP_MOV_MEM64_A,
            _ => OP_MOV_MEM32_A,
        };
