...
use self::sse::{
    lift_cvt, lift_movq, lift_pext_pins, lift_pinsrw, lift_sse, lift_sse_fp, lift_sse_logic,
    lift_sseshift_imm8, lift_sseshuffle, lift_tzcnt, lift_unpcklps,
};
...
fn two_regs(inst: &iced_x86::Instruction) -> Result<(u8, u8)> {
    if inst.op0_kind() != OpKind::Register || inst.op1_kind() != OpKind::Register {
        return Err(anyhow!(
            "lifter: expected register-register form, got {}",
            inst
        ));
    }
...
pub fn lift_one(b: &mut BytecodeBuilder, inst: &Instruction) -> Result<()> {
    let code = inst.code();
    use iced_x86::Code::*;
    ...
    match code {
        ...
        Add_rm8_r8 | Add_r8_rm8 | Add_rm16_r16 | Add_r16_rm16 | Add_AL_imm8 | Add_AX_imm16
        | Add_rm8_imm8 | Add_rm16_imm8 | Add_rm16_imm16 | Sub_rm8_r8 | Sub_r8_rm8
        | Sub_rm16_r16 | Sub_r16_rm16 | Sub_AL_imm8 | Sub_AX_imm16 | Sub_rm8_imm8
        | Sub_rm16_imm8 | Sub_rm16_imm16 | Xor_rm8_r8 | Xor_r8_rm8 | Xor_rm16_r16
        | Xor_r16_rm16 | Xor_AL_imm8 | Xor_AX_imm16 | Xor_rm8_imm8 | Xor_rm16_imm8
        | Xor_rm16_imm16 | And_rm8_r8 | And_r8_rm8 | And_rm16_r16 | And_r16_rm16 | And_AL_imm8
        | And_AX_imm16 | And_rm8_imm8 | And_rm16_imm8 | And_rm16_imm16 | Or_rm8_r8 | Or_r8_rm8
        | Or_rm16_r16 | Or_r16_rm16 | Or_AL_imm8 | Or_AX_imm16 | Or_rm8_imm8 | Or_rm16_imm8
        | Or_rm16_imm16 => lift_narrow_arith(b, inst)?,
        ...
        Mov_r16_imm16 | Mov_r8_imm8 => {
            let r = vreg(inst.op0_register())?;
            let v = if matches!(code, Mov_r8_imm8) {
                inst.immediate8() as u32
            } else {
                inst.immediate16() as u32
            };
            b.mov_r_imm32(r, v);
        }
        ...
        Mov_rm8_imm8 | Mov_rm16_imm16 | Mov_rm32_imm32 | Mov_rm64_imm32 => {
            if inst.op0_kind() == OpKind::Register {
                ...
                return Err(anyhow!(
                    "lifter: mov-imm requires register/memory dest, got {}",
                    inst
                ));
            } else {
                let addr = mem_emit(b, inst, 0)?;
                ...
                let v = if is16 {
                    inst.immediate16() as u64
                } else {
                    inst_imm(inst, is8) as u64
                };
        ...
        Movzx_r32_rm8 | Movzx_r32_rm16 | Movzx_r64_rm8 | Movzx_r64_rm16 => {
            ...
            if inst.op1_kind() == OpKind::Register {
                ...
                b.mov_r_r(d, s);
                let mask = match code {
                    Movzx_r32_rm8 | Movzx_r64_rm8 => 0xFFu32,
                    _ => 0xFFFFu32,
                };
                b.binop_r_imm32(OP_AND_R_IMM32, d, mask);
            } else {
        ...
        Movsx_r64_rm8 | Movsx_r64_rm16 | Movsx_r32_rm8 | Movsx_r32_rm16 => {
            let d = vreg(inst.op0_register())?;
            let src_bits = if matches!(code, Movsx_r64_rm8 | Movsx_r32_rm8) {
                8
            } else {
                16
            };
        ...
        Add_r32_rm32 | Add_r64_rm64 | Add_rm32_r32 | Add_rm64_r64 => {
            two_op(b, inst, OP_ADD_R_R, OP_ADD_R_R64)?
        }
        Sub_r32_rm32 | Sub_r64_rm64 | Sub_rm32_r32 | Sub_rm64_r64 => {
            two_op(b, inst, OP_SUB_R_R, OP_SUB_R_R64)?
        }
        Xor_r32_rm32 | Xor_r64_rm64 | Xor_rm32_r32 | Xor_rm64_r64 => {
            two_op(b, inst, OP_XOR_R_R, OP_XOR_R_R64)?
        }
        And_r32_rm32 | And_r64_rm64 | And_rm32_r32 | And_rm64_r64 => {
            two_op(b, inst, OP_AND_R_R, OP_AND_R_R64)?
        }
        Or_r32_rm32 | Or_r64_rm64 | Or_rm32_r32 | Or_rm64_r64 => {
            two_op(b, inst, OP_OR_R_R, OP_OR_R_R64)?
        }
        ...
        Mul_rm8 | Mul_rm16 | Mul_rm32 | Mul_rm64 | Imul_rm8 | Imul_rm16 | Imul_rm32 | Imul_rm64
        | Div_rm8 | Div_rm16 | Div_rm32 | Div_rm64 | Idiv_rm8 | Idiv_rm16 | Idiv_rm32
        | Idiv_rm64 => lift_muldiv(b, inst)?,
        Bswap_r32 | Bswap_r64 => {
            let r = vreg(inst.op0_register())?;
            let op = if matches!(code, Bswap_r32) {
                OP_BSWAP_R32
            } else {
                OP_BSWAP_R64
            };
        ...
        Add_EAX_imm32 | Add_RAX_imm32 | Add_rm32_imm8 | Add_rm32_imm32 | Add_rm64_imm8
        | Add_rm64_imm32 | Sub_EAX_imm32 | Sub_RAX_imm32 | Sub_rm32_imm8 | Sub_rm32_imm32
        | Sub_rm64_imm8 | Sub_rm64_imm32 | And_EAX_imm32 | And_RAX_imm32 | And_rm32_imm8
        | And_rm32_imm32 | And_rm64_imm8 | And_rm64_imm32 | Or_EAX_imm32 | Or_RAX_imm32
        | Or_rm32_imm8 | Or_rm32_imm32 | Or_rm64_imm8 | Or_rm64_imm32 | Xor_EAX_imm32
        | Xor_RAX_imm32 | Xor_rm32_imm8 | Xor_rm32_imm32 | Xor_rm64_imm8 | Xor_rm64_imm32 => {
        ...
        Shld_rm16_r16_imm8 | Shld_rm32_r32_imm8 | Shld_rm64_r64_imm8 | Shld_rm16_r16_CL
        | Shld_rm32_r32_CL | Shld_rm64_r64_CL | Shrd_rm16_r16_imm8 | Shrd_rm32_r32_imm8
        | Shrd_rm64_r64_imm8 | Shrd_rm16_r16_CL | Shrd_rm32_r32_CL | Shrd_rm64_r64_CL => {
            lift_shld_shrd(b, inst)?;
        }
        Inc_rm32 | Inc_rm64 | Inc_rm8 | Inc_rm16 | Dec_rm32 | Dec_rm64 | Dec_rm8 | Dec_rm16 => {
            lift_incdec(b, inst)?
        }
        ...
        Cld => b.cld(),
        Std => b.std(),
        Xchg_rm64_r64 | Xchg_rm32_r32 | Xchg_r64_RAX | Xchg_r32_EAX | Xchg_rm8_r8
        | Xchg_rm16_r16 => lift_xchg(b, inst)?,
        Imul_r32_rm32_imm8 | Imul_r32_rm32_imm32 | Imul_r64_rm64_imm8 | Imul_r64_rm64_imm32 => {
            lift_imul_imm(b, inst)?
        }
        Cmp_EAX_imm32 | Cmp_RAX_imm32 | Cmp_AL_imm8 | Cmp_AX_imm16 | Cmp_rm32_imm8
        | Cmp_rm32_imm32 | Cmp_rm64_imm8 | Cmp_rm64_imm32 | Cmp_rm32_r32 | Cmp_rm64_r64
        | Cmp_r32_rm32 | Cmp_r64_rm64 | Cmp_rm8_imm8 | Cmp_rm16_imm8 | Cmp_rm16_imm16
        | Cmp_rm8_r8 | Cmp_r8_rm8 | Cmp_r16_rm16 | Cmp_rm16_r16 => {
            lift_cmp(b, inst)?;
        }
        Test_rm32_r32 => {
        ...
        Pushq_imm8 | Pushq_imm32 => {
            ...
            let v = if is8 {
                (inst.immediate8() as i8) as i64 as u64
            } else {
                (inst.immediate32() as i32) as i64 as u64
            };
        ...
        Seto_rm8 | Setno_rm8 | Setb_rm8 | Setae_rm8 | Sete_rm8 | Setne_rm8 | Setbe_rm8
        | Seta_rm8 | Sets_rm8 | Setns_rm8 | Setp_rm8 | Setnp_rm8 | Setl_rm8 | Setge_rm8
        | Setle_rm8 | Setg_rm8 => lift_setcc(b, inst)?,
        Cmove_r32_rm32 | Cmove_r64_rm64 | Cmovne_r32_rm32 | Cmovne_r64_rm64 | Cmovb_r32_rm32
        | Cmovb_r64_rm64 | Cmovae_r32_rm32 | Cmovae_r64_rm64 | Cmovbe_r32_rm32
        | Cmovbe_r64_rm64 | Cmova_r32_rm32 | Cmova_r64_rm64 | Cmovl_r32_rm32 | Cmovl_r64_rm64
        | Cmovge_r32_rm32 | Cmovge_r64_rm64 | Cmovle_r32_rm32 | Cmovle_r64_rm64
        | Cmovg_r32_rm32 | Cmovg_r64_rm64 | Cmovs_r32_rm32 | Cmovs_r64_rm64 | Cmovns_r32_rm32
        | Cmovns_r64_rm64 | Cmovo_r32_rm32 | Cmovo_r64_rm64 | Cmovno_r32_rm32 | Cmovno_r64_rm64
        | Cmovp_r32_rm32 | Cmovp_r64_rm64 | Cmovnp_r32_rm32 | Cmovnp_r64_rm64 => {
            lift_cmovcc(b, inst)?
        }
        Sbb_rm32_r32 | Sbb_r32_rm32 | Sbb_rm32_imm8 | Sbb_rm32_imm32 | Sbb_EAX_imm32
        | Sbb_rm64_r64 | Sbb_r64_rm64 | Sbb_rm64_imm8 | Sbb_rm64_imm32 | Sbb_RAX_imm32 => {
            lift_sbb(b, inst)?
        }
        ...
        | Adc_rm8_r8 | Adc_r8_rm8 | Adc_rm16_r16 | Adc_r16_rm16 | Adc_rm8_imm8 | Adc_rm16_imm8
        | Adc_rm16_imm16 | Adc_AL_imm8 | Adc_AX_imm16 => lift_adc(b, inst)?,
        ...
        Cmpxchg_rm8_r8 | Cmpxchg_rm16_r16 | Cmpxchg_rm32_r32 | Cmpxchg_rm64_r64 => {
            lift_cmpxchg(b, inst)?
        }
        ...
        | Mulss_xmm_xmmm32 | Mulsd_xmm_xmmm64 | Divss_xmm_xmmm32 | Divsd_xmm_xmmm64 => {
            lift_sse_fp(b, inst)?
        }
        ...
        | Cvtss2sd_xmm_xmmm32 | Cvtsd2ss_xmm_xmmm64 | Cvttss2si_r32_xmmm32
        | Cvttss2si_r64_xmmm32 | Cvttsd2si_r32_xmmm64 | Cvttsd2si_r64_xmmm64
        | Cvtss2si_r32_xmmm32 | Cvtss2si_r64_xmmm32 | Cvtsd2si_r32_xmmm64 | Cvtsd2si_r64_xmmm64 => {
            lift_cvt(b, inst)?
        }
        ...
        | Xorps_xmm_xmmm128
        | Xorpd_xmm_xmmm128 => lift_sse(b, inst, 0)?,
        ...
        Cpuid => b.cpuid(),
        Xgetbv => b.xgetbv(),
        Pshuflw_xmm_xmmm128_imm8 | Pshufhw_xmm_xmmm128_imm8 | Pshufd_xmm_xmmm128_imm8 => {
            lift_sseshuffle(b, inst)?
        }
        ...
        Ud2 => b.halt(),
        Bt_rm32_r32 | Bt_rm64_r64 | Bt_rm16_r16 | Bt_rm32_imm8 | Bt_rm64_imm8 | Bt_rm16_imm8 => {
            lift_bt(b, inst)?
        }
        Bts_rm64_r64 | Bts_rm32_r32 | Bts_rm16_r16 | Bts_rm64_imm8 | Bts_rm32_imm8
        | Bts_rm16_imm8 | Btr_rm64_r64 | Btr_rm32_r32 | Btr_rm16_r16 | Btr_rm64_imm8
        | Btr_rm32_imm8 | Btr_rm16_imm8 | Btc_rm64_r64 | Btc_rm32_r32 | Btc_rm16_r16
        | Btc_rm64_imm8 | Btc_rm32_imm8 | Btc_rm16_imm8 => lift_bts(b, inst)?,
        Int_imm8 => b.halt(),
        ...
        | VEX_Blsmsk_r64_rm64
        | VEX_Blsi_r32_rm32
        | VEX_Blsi_r64_rm64 => lift_bmi(b, inst)?,
...
mod tests {
    ...
    fn test_lift_movsx_reg_reg_sign_extends() {
        ...
        assert!(
            dis.contains("sar64") || dis.contains("sar"),
            "movsx reg-reg must sign-extend; disasm:\n{dis}"
        );
    }
...
pub(super) fn lift_bmi(b: &mut BytecodeBuilder, inst: &Instruction) -> Result<()> {
    ...
    if inst.op1_kind() == OpKind::Register {
        ...
        match code {
            ...
            VEX_Andn_r32_r32_rm32 => {
                if inst.op2_kind() != OpKind::Register {
                    return Err(anyhow!(
                        "lifter: andn requires register operands, got {}",
                        inst
                    ));
                }
        ...
        return Err(anyhow!(
            "lifter: BMI memory-source form not yet supported: {}",
            inst
        ));
    }
    Ok(())
