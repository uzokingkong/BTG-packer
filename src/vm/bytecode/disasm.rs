...
pub enum DisasmError {
    #[error("unknown bytecode opcode 0x{opcode:02X} at offset 0x{at:X}")]
    UnknownOpcode { opcode: u8, at: usize },
    #[error(
        "truncated bytecode instruction 0x{opcode:02X} at offset 0x{at:X}: need {needed} operand byte(s), have {available}"
    )]
...
/// Strict disassembly entry point for untrusted or externally supplied streams.
/// It validates every instruction against the canonical opcode registry before
/// any operand indexing occurs.
pub fn try_disassemble(code: &[u8]) -> Result<String, DisasmError> {
    ...
    while ip < code.len() {
        ...
        let op = code[ip];
        ip += 1;
        let operand_len = opcode_operand_len(op).ok_or(DisasmError::UnknownOpcode {
            opcode: op,
            at: start,
        })?;
        let available = code.len().saturating_sub(ip);
        ...
        match op {
            ...
            OP_ROL_R_IMM8 | OP_ROR_R_IMM8 => {
                line += &format!(
                    "{} v{}, {}",
                    if op == OP_ROL_R_IMM8 { "rol" } else { "ror" },
                    code[ip],
                    code[ip + 1]
                );
                ip += 2;
            ...
            OP_ADD_R_IMM64 | OP_XOR_R_IMM64 | OP_AND_R_IMM64 => {
                ...
                ip += 5;
                let m = match op {
                    OP_ADD_R_IMM64 => "add64",
                    OP_XOR_R_IMM64 => "xor64",
                    _ => "and64",
                };
                line += &format!("{} v{}, 0x{:X}", m, r, imm);
            }
            ...
            | OP_MOV_R_MEM64 => {
                let (m, w) = match op {
                    OP_MOVZX_R_MEM16 => ("movzx", 16),
                    OP_MOVZX_R_MEM32 => ("movzx", 32),
                    OP_MOVSX_R_MEM8 => ("movsx", 8),
                    OP_MOVSX_R_MEM16 => ("movsx", 16),
                    _ => ("mov", 64),
                };
                line += &format!(
                    "{} v{}, mem[{}][v{}] (u{})",
            ...
            OP_LEA => {
                ...
                line += &format!(
                    ...
                    base,
                    1i32 << sc,
                    if idx == ADDR_NO_INDEX { 0 } else { idx },
                    disp
                );
            }
            OP_SET_RIP => {
            ...
            | OP_MOVSX_R_MEM16_A | OP_MOV_R_MEM64_A => {
                let (m, w) = match op {
                    OP_MOVZX_R_MEM8_A => ("movzx", 8),
                    OP_MOVZX_R_MEM16_A => ("movzx", 16),
                    OP_MOVZX_R_MEM32_A => ("movzx", 32),
                    OP_MOVSX_R_MEM8_A => ("movsx", 8),
                    OP_MOVSX_R_MEM16_A => ("movsx", 16),
                    _ => ("mov", 64),
                };
            ...
            OP_SHL64_R_IMM8 | OP_SHR64_R_IMM8 | OP_SAR64_R_IMM8 => {
                ...
                ip += 2;
                let m = match op {
                    OP_SHL64_R_IMM8 => "shl64",
                    OP_SHR64_R_IMM8 => "shr64",
                    _ => "sar64",
                };
                line += &format!("{} v{}, {}", m, r, n);
            }
            ...
            OP_NOP => line += "nop",
            OP_MOVSD_XMM_MEM => {
                line += &format!("movsd xmm{}, [v{}]", code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_MOVSD_MEM_XMM => {
                line += &format!("movsd [v{}], xmm{}", code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_MOVUPS_XMM_MEM => {
                line += &format!("movups xmm{}, [v{}]", code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_MOVUPS_MEM_XMM => {
                line += &format!("movups [v{}], xmm{}", code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_UNPCKLPD_XMM => {
                line += &format!("unpcklpd xmm{}, xmm{}", code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_UNPCKLPS_XMM => {
                line += &format!("unpcklps xmm{}, xmm{}", code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_XORPS_XMM => {
                line += &format!("xorps xmm{}, xmm{}", code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_PSHUFLW_XMM => {
                line += &format!(
                    "pshuflw xmm{}, xmm{}, 0x{:02X}",
                    code[ip],
                    code[ip + 1],
                    code[ip + 2]
                );
            ...
            OP_MOVQ_XMM_GPR => {
                line += &format!("movq v{}, xmm{}", code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_MOVQ_GPR_XMM => {
                line += &format!("movq xmm{}, v{}", code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_PSRLQ_XMM_IMM8 => {
                line += &format!("psrlq xmm{}, 0x{:02X}", code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_PSLLQ_XMM_IMM8 => {
                line += &format!("psllq xmm{}, 0x{:02X}", code[ip], code[ip + 1]);
                ip += 2;
            }
            // ── v54: SSE/FPU (Group A) ──────────────────────────────────────
            OP_ADDSS_XMM | OP_ADDSD_XMM | OP_SUBSS_XMM | OP_SUBSD_XMM | OP_MULSS_XMM
            | OP_MULSD_XMM | OP_DIVSS_XMM | OP_DIVSD_XMM => {
                let m = match op {
                    OP_ADDSS_XMM => "addss",
                    OP_ADDSD_XMM => "addsd",
                    OP_SUBSS_XMM => "subss",
                ...
                line += &format!("{} xmm{}, xmm{}", m, code[ip], code[ip + 1]);
                ip += 2;
            }
            ...
            OP_CVTSI2SD_XMM | OP_CVTSI2SS_XMM => {
                let m = if op == OP_CVTSI2SD_XMM {
                    "cvtsi2sd"
                } else {
                    "cvtsi2ss"
                };
                line += &format!("{} xmm{}, v{}", m, code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_CVTSS2SD_XMM | OP_CVTSD2SS_XMM => {
                let m = if op == OP_CVTSS2SD_XMM {
                    "cvtss2sd"
                } else {
                    "cvtsd2ss"
                };
            ...
            OP_CVTTSS2SI | OP_CVTTSD2SI | OP_CVTSS2SI | OP_CVTSD2SI => {
                ...
                line += &format!("{} v{}, xmm{}", m, code[ip], code[ip + 1]);
                ip += 2;
            }
            OP_PEXTRD_XMM => {
                line += &format!(
            ...
            OP_LOCK_INC_MEM8_A | OP_LOCK_INC_MEM16_A | OP_LOCK_INC_MEM32_A
            | OP_LOCK_INC_MEM64_A => {
                let w = match op {
                    OP_LOCK_INC_MEM8_A => 8,
                    OP_LOCK_INC_MEM16_A => 16,
                    OP_LOCK_INC_MEM32_A => 32,
                    _ => 64,
                };
                line += &format!("lock inc{} [v{}]", w, code[ip]);
                ip += 1;
            }
            OP_LOCK_DEC_MEM8_A | OP_LOCK_DEC_MEM16_A | OP_LOCK_DEC_MEM32_A
            | OP_LOCK_DEC_MEM64_A => {
                let w = match op {
                    OP_LOCK_DEC_MEM8_A => 8,
                    OP_LOCK_DEC_MEM16_A => 16,
                    OP_LOCK_DEC_MEM32_A => 32,
                    _ => 64,
                };
                line += &format!("lock dec{} [v{}]", w, code[ip]);
                ip += 1;
            }
            // ── v31: multiply/divide + BSWAP ──────────────────────────────
            OP_MUL_R_R32 | OP_MUL_R_R64 => {
                let w = if op == OP_MUL_R_R32 { "mul32" } else { "mul64" };
                line += &format!("{} v{}", w, code[ip]);
                ip += 1;
            }
            OP_IMUL1_R_R32 | OP_IMUL1_R_R64 => {
                let w = if op == OP_IMUL1_R_R32 {
                    "imul32"
                } else {
                    "imul64"
                };
...
/// Best-effort diagnostic wrapper retained for existing debug and assertion
/// call sites. Malformed input is rendered as an explicit error instead of
/// panicking on an unchecked slice.
pub fn disassemble(code: &[u8]) -> String {
    try_disassemble(code).unwrap_or_else(|e| format!("<bytecode decode error: {e}>\n"))
}
...
mod strict_tests {
    ...
    fn truncated_instruction_is_an_error_not_a_panic() {
        let full = [OP_MOV_R_IMM64, 3, 1, 2, 3, 4, 5, 6, 7, 8];
        for end in 1..full.len() {
            assert!(
                matches!(
                    try_disassemble(&full[..end]),
                    Err(DisasmError::TruncatedInstruction { .. })
                ),
                "prefix length {end} must be rejected"
            );
        }
        assert!(try_disassemble(&full).is_ok());
    }

    #[test]
    fn unknown_opcode_is_an_error_not_a_panic() {
        assert!(matches!(
            try_disassemble(&[0xFF]),
