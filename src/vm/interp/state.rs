...
//
// ── 멀티스레드 진입 경계 (아이템 9) ───────────────────────────────────────────
// 이 `state` 버퍼는 per-invocation 이다 — `interp::interpret(state, mem, code)` 가
// state 를 인자로 받으므로, 스레드가 각자 독립 버퍼를 쓰면 동시 진입에 재진입
// 안전하다 (mt_reentrancy self-test 가 결정성을 고정). 반면 패킹된 배포 런타임의
// 프로그램 VM 은 **단일 정적 state 버퍼**를 쓰므로, 멀티스레드 프로그램에서
// 가상화 함수를 동시에 진입하면 한 버퍼를 공유해 손상된다. 배포 런타임이
// 멀티스레드를 지원하려면 스레드당 독립 state (TLS 기반) 가 필요하며, 이 모듈의
// 접근자가 그 참조 모델이 된다.
// ==============================================================================
