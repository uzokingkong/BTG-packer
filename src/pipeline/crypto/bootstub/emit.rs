...
pub(crate) fn emit_base_bind_loop(seq: &mut Vec<(Instruction, Option<Label>)>, seed_va: u64) {
    ...
    // Real data dependency (junk -> real): prologue trashformer_mixing_loop left its
    // seed-derived mixed accumulator in EDX. Capture into R9D before EDX is clobbered,
    // then fold into the bind byte twice (compensated, net 0) so the real base-bind path
    // genuinely depends on the junk -- cannot be dropped by live-range analysis.
    seq.push((
    ...
    // junk accumulator via R9D -- real def-use, cannot drop the junk by liveness).
    seq.push((
        Instruction::with2(Code::Xor_rm32_r32, Register::R8D, Register::R9D).unwrap(),
        None,
    ));
    seq.push((
        Instruction::with2(Code::Xor_rm32_r32, Register::R8D, Register::R9D).unwrap(),
    ...
    seq.push((
        Instruction::with2(Code::Mov_r64_imm64, Register::RSI, seed_va).unwrap(),
        None,
    ));
    seq.push((
        Instruction::with2(Code::Mov_r32_imm32, Register::EDI, 0x100).unwrap(),
        None,
    ));
    seq.push((
        Instruction::with2(Code::Xor_rm8_r8, M::with_base(Register::RSI), Register::R8L).unwrap(),
        Some(Label::BaseBindLoop),
    ));
    seq.push((
    ...
    seq.push((
        Instruction::with1(Code::Dec_rm32, Register::EDI).unwrap(),
        None,
    ));
    seq.push((
...
/// M12 Decrypt-Descriptor: 부트 스텁이 코드/런/바이트코드 복호화에 쓰는 타깃/크기/
/// 바이트코드/테이블 주소를 디스크립터(파생 키 = RC4 keystream으로 암호화)에서 런타임에
/// 복호화해 쓴다. 정적 분석 시 부트 스텁 imm64에서 decrypt 주소가 노출되지 않는다.
/// (패커가 `Rc4::new(seed_masked)`로 암호화 — 여기서 동일 KSA(seed)+canonical PRGA로
///  복호화한다.) RBX = S-box base(=RSP, 스택 프레임). 이 스테이지의 S-box는 즉시 뒤
///  main KSA-init이 덮어쓰므로 일시적이다.
pub(crate) fn emit_desc_decrypt(seq: &mut Vec<(Instruction, Option<Label>)>, stub: &BootStubCtx) {
    // S-box base = stack (스택 프레임 — RBX는 이미 RSP지만 명시적으로 재확정)
    seq.push((
        Instruction::with2(Code::Mov_r64_rm64, Register::RBX, Register::RSP).unwrap(),
        None,
    ));
    // raw KSA(key = seed_va 256B) → S-box = KSA(seed_masked)
    seq.push((
    ...
    // canonical RC4 PRGA: i=0, j=0에서 시작 (Rc4::new start와 동일)
    seq.push((
    ...
    // 디스크립터 in-place 복호화
    seq.push((
        Instruction::with2(Code::Mov_r64_imm64, Register::RCX, stub.desc_va).unwrap(),
        None,
    ));
    seq.push((
        Instruction::with2(Code::Mov_r64_imm64, Register::RDX, stub.desc_size as u64).unwrap(),
        None,
    ));
    seq.push((
        Instruction::with_branch(Code::Call_rel32_64, 0).unwrap(),
...
pub(crate) fn trashformer_junk(seed: u32) -> Vec<Instruction> {
    ...
    for _ in 0..n {
        ...
        match next() % 5 {
            ...
            _ => out
                .push(Instruction::with2(Code::Lea_r64_m, a, MemoryOperand::with_base(b)).unwrap()),
        }
    }
    out
}
...
/// v17+ (junk rework): TrashFormer junk hardened with a REAL-working ARX mixing
/// loop. Simple dead-register ops alone let an analyst instantly label them as junk;
/// this loop is structurally identical to a real checksum/decrypt loop, and its
/// iteration count is seed-derived, ALWAYS NONZERO (1..256), so it never reads as a
/// "dead path entered with length=0 / fake path" — fake vs real become statically
/// indistinguishable. (The S-box frame [RSP..RSP+count) is fully re-initialized by
/// KSA before first use, so pre-filling it is safe. rax/rcx/rdi are not live yet.)
pub(crate) fn trashformer_mixing_loop(seed: u32) -> Vec<(Instruction, Option<Label>)> {
    let count = 1 + (seed % 0x100); // 1..256 — always nonzero, within frame
    let acc = seed.wrapping_mul(0x9E37_79B9) | 1; // build-derived nonzero mix accumulator
    let mut v = Vec::with_capacity(8);
    v.push((
        Instruction::with2(Code::Mov_r32_imm32, Register::ECX, count).unwrap(),
        None,
    ));
    v.push((
        Instruction::with2(Code::Mov_r64_rm64, Register::RDI, Register::RSP).unwrap(),
        None,
    ));
    v.push((
        Instruction::with2(Code::Mov_r32_imm32, Register::EAX, acc).unwrap(),
        None,
    ));
    // JunkMixLoop: [rdi] ^= al ; rol eax,5 ; eax += ecx ; inc rdi ; dec ecx ; jnz
    v.push((
        Instruction::with2(
            Code::Xor_rm8_r8,
        ...
        .unwrap(),
        Some(Label::JunkMixLoop),
    ));
    v.push((
        Instruction::with2(Code::Rol_rm32_imm8, Register::EAX, 5).unwrap(),
        None,
    ));
    v.push((
        Instruction::with2(Code::Add_rm32_r32, Register::EAX, Register::ECX).unwrap(),
        None,
    ));
    v.push((
    ...
    v.push((
        ...
        None,
    )); // leave mixed accumulator in EDX (real junk→real dep below)
    v
}
...
pub(crate) fn emit_c1_init(seq: &mut Vec<(Instruction, Option<Label>)>, stub: &BootStubCtx) {
    ...
    seq.push((
        Instruction::with2(
            Code::Mov_r32_rm32,
            Register::R8D,
            M::with_base_displ(Register::RSI, 32),
        )
        .unwrap(),
        None,
    ));
...
pub(crate) fn emit_ksa_init(seq: &mut Vec<(Instruction, Option<Label>)>, stub: &BootStubCtx) {
    ...
    if !stub.no_crypto {
        if stub.chained {
            ...
            seq.push((
                Instruction::with2(Code::Mov_r64_imm64, Register::RDX, stub.seed_va).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with2(Code::Mov_r64_imm64, Register::R8, stub.c1_state_va).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with2(Code::Mov_r64_imm64, Register::RCX, stub.vm_state_va).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with_branch(Code::Call_rel32_64, stub.vm_entry_va).unwrap(),
                None,
            ));
        } else if stub.vm {
            // ── v3-composite VM 경로 ──────────────────────────────────────────────
            ...
            // 3곳이 어긋날 위험이 있었다.)
            for item in
                crate::vm::ksa::build_ksa_instructions(stub.seed_va, stub.k1, stub.k2, stub.k3)
            {
                let lbl = item
                    .label
                    .map(map_ksa_label)
...
pub(crate) fn emit_code_decrypt(seq: &mut Vec<(Instruction, Option<Label>)>, stub: &BootStubCtx) {
    ...
    if !stub.no_crypto {
        if stub.chained {
            ...
            seq.push((
                Instruction::with2(Code::Xor_r32_rm32, Register::R12D, Register::R12D).unwrap(),
                None,
            ));
            seq.push((
            ...
            seq.push((
                ...
                Some(Label::ChainLoop),
            ));
            seq.push((
                Instruction::with_branch(Code::Je_rel32_64, 0).unwrap(),
                Some(Label::ChainDone),
            ));
            ...
            seq.push((
                Instruction::with2(Code::Add_rm64_r64, Register::RCX, Register::R12).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with2(Code::Sub_rm64_imm32, Register::RCX, 0x100).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with2(Code::Test_rm64_r64, Register::R12, Register::R12).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with_branch(Code::Jne_rel32_64, 0).unwrap(),
            ...
            seq.push((
                Instruction::with2(Code::Cmp_rm64_imm32, Register::R13, 0x100).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with2(Code::Cmovb_r64_rm64, Register::R14, Register::R13).unwrap(),
                None,
            ));
            // 청크 포인터 = code_va + off
            seq.push((
            ...
            seq.push((
                Instruction::with2(Code::Sub_rm64_r64, Register::R13, Register::R14).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with_branch(Code::Jmp_rel32_64, 0).unwrap(),
            ...
            seq.push((
                Instruction::with2(Code::Cmp_rm64_imm32, Register::R11, 0x100).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with_branch(Code::Jb_rel32_64, 0).unwrap(),
                Some(Label::StrKeyUseWindow),
            ));
            seq.push((
                Instruction::with2(Code::Mov_r64_imm64, Register::RAX, stub.desc_va).unwrap(),
                None,
...
pub(crate) fn emit_run_decrypt(seq: &mut Vec<(Instruction, Option<Label>)>, stub: &BootStubCtx) {
    ...
    // S1 runtime-derived poison interlock: emit_integrity_crc(site 1) has left
    // V1 = computed ^ stored in R14. On a legit build V1=0 -> this loop leaves the
    // run bytes untouched; on tamper V1!=0 -> even a single-byte patch of the site-1
    // `je` cannot bypass, because R14!=0 rotates through 4 bytes (ror 8) and corrupts
    // every run/import-resolve table on the way out (single-byte-patch defeat).
    if stub.integrity {
        seq.push((
            Instruction::with2(
                Code::Mov_r64_rm64,
        ...
        seq.push((
            Instruction::with2(Code::Mov_r32_rm32, Register::R9D, Register::R14D).unwrap(),
            None,
        )); // rotating poison key
        seq.push((
        ...
        seq.push((
            Instruction::with2(Code::Ror_rm32_imm8, Register::R9D, 8).unwrap(),
            None,
        ));
        seq.push((
    ...
    seq.push((
        Instruction::with1(Code::Dec_rm64, Register::R11).unwrap(),
        None,
    ));
    seq.push((
        Instruction::with_branch(Code::Jmp_rel32_64, 0).unwrap(),
    ...
    seq.push((
        Instruction::with2(Code::Xor_r32_rm32, Register::R11D, Register::R11D).unwrap(),
        Some(Label::RunDone),
    ));
}
    ...
    if !stub.no_crypto {
        ...
        seq.push((
            Instruction::with2(
                Code::Mov_r64_rm64,
                Register::R11,
                MemoryOperand::with_base_displ(Register::RAX, 0x38),
            )
            .unwrap(),
            None,
        ));
    ...
    if !stub.no_crypto {
        ...
        seq.push((
            Instruction::with2(
                Code::Mov_r64_rm64,
                Register::RDX,
                MemoryOperand::with_base_displ(Register::RAX, 0x28),
            )
            .unwrap(),
            None,
        ));
        ...
        seq.push((
            Instruction::with2(Code::Mov_r64_imm64, Register::RCX, stub.vm_oep_bc_va).unwrap(),
            None,
        ));
        seq.push((
            Instruction::with2(
...
pub(crate) fn emit_self_wipe(seq: &mut Vec<(Instruction, Option<Label>)>, stub: &BootStubCtx) {
    ...
    if !stub.no_crypto {
        ...
        seq.push((
            Instruction::with2(Code::Mov_r64_rm64, Register::RCX, Register::RSP).unwrap(),
            None,
        ));
        seq.push((
            Instruction::with2(Code::Mov_r64_imm64, Register::RDX, 0x100).unwrap(),
        ...
        if stub.c1_mode() {
            seq.push((
                Instruction::with2(Code::Mov_r64_imm64, Register::RCX, stub.c1_state_va).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with2(Code::Mov_r64_imm64, Register::RDX, 0x80).unwrap(),
                None,
            ));
            seq.push((
                Instruction::with_branch(Code::Call_rel32_64, 0).unwrap(),
...
pub(crate) fn emit_dispatcher_entry(
    ...
    seq.push((
        Instruction::with2(Code::Add_rm64_imm32, Register::RSP, stub.stack_frame).unwrap(),
        None,
    ));
    if stub.vm_oep {
        ...
        use iced_x86::MemoryOperand as M;
        seq.push((
            Instruction::with2(Code::Mov_r64_imm64, Register::RAX, stub.vm_prog_state_va).unwrap(),
            None,
        ));
        seq.push((
            Instruction::with2(
        ...
        if stub.vm_oep_native_entry {
            ...
            seq.push((
                Instruction::with_branch(Code::Jmp_rel32_64, stub.vm_oep_native_va).unwrap(),
                None,
            ));
        } else {
            // 프로그램 VM: state 포인터를 RCX로 전달하고 프로그램 VM 엔트리로 점프.
            seq.push((
                Instruction::with2(Code::Mov_r64_imm64, Register::RCX, stub.vm_prog_state_va)
                    .unwrap(),
                None,
            ));
            seq.push((
                Instruction::with2(Code::Mov_r64_imm64, Register::RAX, stub.vm_prog_entry_va)
                    .unwrap(),
                None,
            ));
            seq.push((
        ...
        seq.push((
            Instruction::with1(Code::Pushq_imm32, stub.entry_block_id as i32).unwrap(),
            None,
        ));
        // ── S4 feedforward (--integrity): entry_seed ^= V4 (사이트4 tamper 결과, EAX).
        //    분기 없이 항상 수행 → 사이트의 `je`→`jmp` 단일 바이트 패치로는 무력화 불가.
        //    정상 실행에서 V4==0이므로 push된 seed는 원래값과 동일; 코드 변조 시 V4!=0
        //    → 디스패처가 entry_seed로 파생하는 블록 키가 잘못되어 쓰레기 복호화/크래시.
        //    (mov edx,imm는 zero-extend; movsxd로 pushq_imm32의 sign-extend 의미 보존.)
        if stub.integrity {
            seq.push((
                Instruction::with2(Code::Mov_r32_imm32, Register::EDX, stub.entry_seed).unwrap(),
                None,
            ));
            seq.push((
            ...
            seq.push((
                Instruction::with1(Code::Pushq_imm32, stub.entry_seed as i32).unwrap(),
                None,
            ));
        }
        seq.push((
            Instruction::with2(Code::Mov_r64_imm64, Register::RAX, stub.dispatcher_va).unwrap(),
            None,
        ));
        seq.push((
            Instruction::with1(Code::Jmp_rm64, Register::RAX).unwrap(),
