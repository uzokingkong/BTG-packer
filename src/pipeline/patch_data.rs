...
fn is_rva_range_protected(start_rva: u32, len: u32, protected_ranges: &[(u32, u32)]) -> bool {
...
fn rva_to_slice<'a>(sections: &'a [SectionData], rva: u32) -> Option<&'a [u8]> {
    for sec in sections {
...
fn get_ascii_string_rva_range(sections: &[SectionData], rva: u32) -> Option<(u32, u32)> {
    if rva == 0 {
        return None;
    }
...
fn collect_import_directory_ranges(
...
fn collect_delay_import_directory_ranges(
    sections: &[SectionData],
    delay_rva: u32,
    delay_size: u32,
    ranges: &mut Vec<(u32, u32)>,
