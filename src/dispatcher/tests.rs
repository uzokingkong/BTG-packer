...
#[test]
    ...
    fn test_dispatcher_obf_level_changes_key_schedule() {
        ...
        assert!(!decode_key_schedule_has_mba_lea(&code1), "level 1 must not contain the 2*(s&b) MBA LEA");
        // level 2: lea eax,[rax+r11*2] 는 있지만 overlap add 는 없어야 한다.
        assert!(decode_key_schedule_has_mba_lea(&code2), "level 2 must contain the 2*(s&b) MBA LEA");
        // level 3: level 2 에 추가로 `add eax, r11d`(overlap) 가 있어야 한다.
        assert!(decode_key_schedule_has_mba_lea(&code3), "level 3 must contain the 2*(s&b) MBA LEA");
        assert!(decode_key_schedule_has_overlap_add(&code3), "level 3 must add (s&b) overlap after the XOR");
    }
    ...
    fn decode_key_schedule_has_overlap_add(code: &[u8]) -> bool {
        ...
        seq.windows(2).any(|w| {
            w[0].0 == Code::Xor_rm32_imm32 && w[1].0 == Code::Add_rm32_r32 && w[1].1 == Register::R11D as u8
        })
    }
