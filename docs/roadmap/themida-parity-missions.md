...
|---|---|---|---|
| 2026-08-18 | — | 미션 보드 등록 | ✅ |
| 2026-08-19 | T3-1 | **Phase B**: BootStubCtx `CryptoMode`(Rc4/C1/ChaCha20) variant + `--crypto-mode chacha20` CLI + bootstub/emit.rs RC4 PRGA 자리에 chacha blob 호출/init 배선 + 패커측 코드·문자열 영역 at-rest 암호화 전환 (seed 원시 파생 `derive_chacha_key_nonce_raw` — 부트 스텁 `emit_chacha_init`과 동일). chained/reencrypt/--vm/--vm-oep 조합은 폴백+경고. `repro/test_prog.exe` pack→run 16-test + FINAL CHECKSUM `0x2cdc0e4511d84a64` 무회귀 (default C1 / `--crypto-mode chacha20` / rc4 / chacha+chained·vm·vm-oep·reencrypt 폴백 전부 green, exit 0). `cargo test --release --lib` 375 passed. ⬜ Phase D: Poly1305 AEAD 무결성 검증 | 🔶 |
| 2026-08-19 | readccc 리뷰 | **팩트체크**: ① `--seed --vm --m8` 결정성 위반(`build_vm_module_mba` `thread_rng`) **실증 확인+수정**(이제 byte-identical, 회귀 테스트 추가) ② 공급망(Cargo.lock/README/LICENSE는 존재하나 CI/SECURITY.md/rust-toolchain 없음) → CI 워크플로우+rust-toolchain.toml+SECURITY.md 추가 ③ ChaCha20-Poly1305 "미통합"은 Phase B로 스트림 at-rest는 통합됨(AEAD tag만 잔여) ④ at-rest→ASLR 비활성, anti-debug ud2/시드변조, whole-program VM 함수 경계·SEH 제한은 소스로 확인됨. **능력 명세**: manifest에 crypto_mode/at_rest/aslr/integrity/coverage 기록. `cargo test --release --lib` 376 passed. ⬜ 잔여: AEAD Phase D, W^X 코드/데이터 분리, anti-debug graceful 정책, 함수 원자성·브리지 ABI 명세 | 🔶 |
