...
| 우선순위 | 작업 | 검증 | 상태 |
|---|---|---|---|
| 🔴 | R1 RIP-relative 활성화 — **게이트 유지 (2026-08-17 재실측)** | 아래 원인 분석 참조 | ⚠️ 구조적 한계 |
| 🔴 | R4 Float op 폴리 ISA/핸들러 추가 (isa_spec 79→86) | 차등 테스트 (3 seeds) | ✅ `3767dff` |
| 🟠 | R6 ADC/SBB, ROL/ROR, SHLD/SHRD, BT계열 | `--text-vm` 100% | ⬜ |
| 🟠 | R7 8/16-bit 논리·시프트·NEG/NOT | 차등 테스트 | ✅ `461e1b4` |
| 🟡 | R2 CMPXCHG reg, R3 하이바이트, R8 packed SSE | 커버리지 매트릭스 | ⬜ |

> **R1 원인 분석 (2026-08-17 재실측)**: 게이트 해제 실험 — (a) `.text`(코드) 타깃만
> 게이트 → 7750 블록 가상화 → **pack→run 갈라짐**(exit 0x400A8D53, main 이전). (b) 코드
> 포인터 슬롯까지 게이트 → 7689 → **여전히 갈라짐**. (c) 전부 게이트(원래 상태) → 6904 →
> **정상**. ⇒ RIP-relative **데이터 절대 VA lift 자체**가 갈라짐의 원인. 원인은 패킹의
> `.rdata` 코드 포인터 **재배치**(원본 .text VA → .textb)와 lift 간 불일치: lift된 블록이
> 재배치된 값을 읽으면 VM이 예상하는 원본 주소와 달라져 네이티브 브리지로 잘못 분기.
> journal의 "함수 원자성 시도 커버리지 절반 + 크래시 미해소"와 일치하는 **구조적 한계**.
> 해결에는 재배치-인식 lift(패킹 후 주소 맵을 lift에 반영)가 필요 — 후속 작업으로 유지.
